...
| 20.4. `mainlog` |
| [Prev](log_formats.fbllog.php)  | Chapter 20. Log Formats |  [Next](log_formats.rejectlog.php) |

## 20.4. `mainlog`

The `mainlog` logs reception, delivery, transient failure, and permanent failure events.
...
### 20.4.1. Reception Records

A reception line is written to the `mainlog` for every reception that SparkPost Elite performs.
...
**Table 20.4. Reception Record Fields**
...
| 9 | 10.0.1.1 | IP address from which the message was received |
...
### 20.4.2. Delivery Records

A delivery line is written to the `mainlog` for every message that is delivered by SparkPost Elite, including transfers between nodes.
...
**Table 20.5. Delivery Record Fields**
...
`D` indicating a successful delivery or `X` indicating a transfer between nodes
...
### 20.4.3. Transient Failure Records

A transient failure line is written to the `mainlog` for every transient failure that occurs. A transient failure is a failure that allows SparkPost Elite to retry sending the message.
...
**Table 20.6. Transient Failure Record Fields**
...
| 7 | group-a | MultiVIP® binding group to which the message was assigned |
| 8 | binding-a | MultiVIP® binding to which the message was assigned |
| 9 | 15 | Stage of the message (See [Section 20.7, “Connection Stages”](log_formats.connection.stages.php "20.7. Connection Stages").) |
| 10 | 0 | 
...
### 20.4.4. Permanent Failure Records

A permanent failure line is written to the `mainlog` for every permanent failure that occurs. A permanent failure indicates that the attempted message failed in such a way that it should not be retried again. After logging a permanent failure, the message will be discarded.
...
**Table 20.7. Permanent Failure Record Fields**
...
| 2 | 00/00-03736-F4101B54 | Batch ID |
| 3 | 00/00-04532-A3456B54 | Connection ID |
| 4 | P | `P` indicating a permanent failure |
| 5 | postalengine.com | Destination domain |
| 6 | 31 | 
...
| 7 | group-a | MultiVIP® binding group to which the message was assigned |
| 8 | binding-a | MultiVIP® binding to which the message was assigned |
| 9 | 5 | Stage of the message (See [Section 20.7, “Connection Stages”](log_formats.connection.stages.php "20.7. Connection Stages").) |
...
### 20.4.5. Heartbeat Records

In addition to the records described in the previous sections, a heartbeat is written periodically to the log, indicating that SparkPost Elite is still active and may log further data.
...
**Table 20.8. Heartbeat Record Fields**
...
| [Prev](log_formats.fbllog.php)  | [Up](log_formats.php) |  [Next](log_formats.rejectlog.php) |
| 20.3. `fbllog`  | [Table of Contents](index.php) |  20.5. `rejectlog` |
