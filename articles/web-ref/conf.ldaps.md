...
| [Prev](conf.aaa.php)  | Chapter 2. Configuration |  [Next](conf.multi-core.php) |
...
If you choose to authenticate against Active Directory, the User Management menu documented at [Section 3.9, “Administering Users From the Web Console”](web3.users.php "3.9. Administering Users From the Web Console") will be removed from the web UI. This menu is used to create, remove or update users and groups.
...
*   Momentum does not support SASL authentication for LDAP. Instead, it has to use the simple authentication scheme, which sends the password in plaintext to the LDAP server.
    ...
    LDAPS queries that don't specify the `x-ldap-version=3` extension are liable to fail. LDAP extensions are discussed in [Section 14.30.3.3, “LDAP”](modules.ds_core.php#modules.ds_core.ds_ldap "14.30.3.3. LDAP").
...
1.  On the cluster manager, update `ecelerity.conf` to use the "LOGIN" authentication scheme, rather than "MD5 digest". Authentication schemes are documented in [Section 2.2, “Authentication, Authorization and Accounting”](conf.aaa.php "2.2. Authentication, Authorization and Accounting"). This is needed so that passwords are passed in plaintext to the LDAP server.
...
4.  Update `webui-common.conf` to specify your Active Directory server:
    ...
    For more information about the Datasource module see [Section 14.30, “ds_core – Datasource Query Core”](modules.ds_core.php "14.30. ds_core – Datasource Query Core").
...
An example `webui-common.conf` file is reproduce in [Appendix D, *Example LDAP `webui-common.conf` File*](webui-common.example.php "Appendix D. Example LDAP webui-common.conf File") .

### 2.3.3. Testing Active Directory Authentication

See [Section 2.3.4, “Debugging Active Directory Authentication”](conf.ldaps.php#conf.ldaps.debug "2.3.4. Debugging Active Directory Authentication") if you have problems with any of the following steps.
...
| [Prev](conf.aaa.php)  | [Up](conf.php) |  [Next](conf.multi-core.php) |
| 2.2. Authentication, Authorization and Accounting  | [Table of Contents](index.php) |  2.4. Configuring for Multiple Event Loops in Momentum 3.6 |
