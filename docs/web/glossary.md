...
A standard format for communicating feedback reports about email abuse.
...
A form of return receipt for email, typically an out-of-band notification of delivery failure. See also [Bounce](glossary.php#gloss-bounce "Bounce").
...
Modules may be loaded in "passive" mode using the 'passive' keyword as shown in the following example:

`Validate validate/cloudmark cloudmark passive { ... }`

The passive option is principally used with Validation modules. Modules loaded in passive mode will **not** have their validation hooks called automatically. Rather, the calls will be driven by a Sieve action associated with the module. For example, the Sieve action 'antivirus' will drive antivirus modules such as the 'clamav' module.
...
An SMTP technique for validating email addresses. Typically, verification is made by attempting to make an SMTP connection to the `mail from` address of an email receipt. See also [Section 14.53, “smtp_cbv – SMTP Callback Verification”](modules.smtp_cbv.php "14.53. smtp_cbv – SMTP Callback Verification").
...
| [Prev](acronyms.php)  |   |   |
| Appendix I. Acronyms  | [Table of Contents](index.php) |   |
