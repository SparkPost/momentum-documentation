...
| [Prev](eccluster.conf3.php)  | Chapter 10. Cluster Configuration Reference |  [Next](cluster.boot.php) |
...
For an in-depth discussion of consolidated cluster logging see [Section 7.6, “Log Aggregation”](cluster.config.logging.php "7.6. Log Aggregation").
...
Each node can respond to normal console commands received on the `control_group`. The cluster console manager utilizes this group to issue cluster-wide configuration commands to update and discover changes in configuration information. For more information about cluster console commands see [Section 7.7.2, “Cluster Management Using Console Commands”](cluster.config.replication.php#cluster.logging.console "7.7.2. Cluster Management Using Console Commands").
...
In addition to native Momentum data, it is possible to replicate user controlled data sets as well (such as caches). This provides a transparent and convenient mechanism to cache data from a module level in a medium that is accessible via every node participating in the cluster. This is discussed in more detail in [Section 7.7, “Data Replication”](cluster.config.replication.php "7.7. Data Replication").
...
In the Topology configuration shown in [Example 10.2, “The `ecelerity-cluster.conf` file”](ecelerity-cluster.conf.php#ecelerity-cluster.conf.example "Example 10.2. The ecelerity-cluster.conf file"), `10.1.1.0/24` informs the clustering module that IPs in the range specified will be added to the `eth1` ethernet interface.
...
Find below a list of all cluster options not covered by the categories discussed above. For a complete list of all cluster options see [Table 9.4, “cluster options”](options-summary.php#cluster-options-table "Table 9.4. cluster options").
...
| [Prev](eccluster.conf3.php)  | [Up](cluster.ref.php) |  [Next](cluster.boot.php) |
| eccluster.conf  | [Table of Contents](index.php) |  cluster.boot |
