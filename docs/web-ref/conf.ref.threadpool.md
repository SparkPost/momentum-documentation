...
If you use the [csapi module](modules.csapi "14.22. csapi – The Content Scanning API Module") be sure to set the concurrency of the CPU threadpool to a value that is less than the max_concurrency of the csapi module.
...
When using the custom_logger module, in some circumstances you may find it necessary to create a thread pool so that you can explicitly define its backlog option. For more information see [Section 14.24, “custom_logger – Customizable Logging”](modules.custom_logger "14.24. custom_logger – Customizable Logging").
